function [SSP_fin,T_fin,S_fin] = calc_ss(TS_data, mesh_data, z_fin)
%
...
%
% Three-dimensional interpolation from input temperature and salinity data
% and further calculation of sound speed (from [1]):
% i) horizontally, layer by layer, to get data for the mesh nodes, by using 
% 'scatteredInterpolant' (MATLAB's built-in function)
% ii) vertically, by using 'interp1' to get data for user's defined vertical
% (z axis) discretization
% 
% References:
% [1] - Mackenzie, K.V. "Nine-term Equation for Sound Speed in the Oceans", J. Acoust. Soc. Am. 70 (1981), 807-812.
% 
%
%%% INPUTS:
% TS_data - struct with temperature and salinity data
% mesh_data - struct with OceanMesh2D mesh data
% z_fin - intended vertical discretization by the user
%
%%% OUTPUTS:
% SSP_fin - final sound speed field
% T_fin - final temperature field
% S_fin - final salinity field
%
